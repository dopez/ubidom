...
function dhtmlXSideBar(conf) {
	...
	this._doOnSideClick = function(e) {
		...
		while (t != null && id == null && e.type != "pointerdown" && e.type != "MSPointerDown") {
			if (typeof(t.className) != "undefined" && t.className.match(/^dhxsidebar_item/) != null && typeof(t._idd) != "undefined") {
				id = t._idd;
			} else {
				t = t.parentNode;
			}
...
dhtmlXSideBar.prototype.setSizes = function() {
	...
	this._adjustSide();
	this._checkHeight();
	this._adjustCell(this.conf.selected); // active cell
...
dhtmlXSideBar.prototype._setItemActive = function(id, callEvent) {
	...
	if (this.conf.selected == id) return;
...
window.dhtmlXSideBarCell = function(id, sidebar) {
	...
	for (var a in this.conf.sidebar_funcs) {
		this[a] = this._sidebarCall(this.conf.sidebar_funcs[a]);
	};
	...
	this.attachEvent("_onBeforeContentAttach", function(dataType) {
		if (dataType == "tabbar" || dataType == "layout" || dataType == "acc" || dataType == "sidebar") {
			this._hideBorders();
		}
