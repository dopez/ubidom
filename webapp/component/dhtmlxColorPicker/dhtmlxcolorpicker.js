...
dhtmlXColorPicker.prototype.setColor = function(value) {
	var rgb, temp, is_check=true, hsl,
	old_value = this.colorAIP.rgb2hex({ r:this.value.red, g:this.value.green, b:this.value.blue }), 
	new_value;
	if (value instanceof Array) {
		...
		if (/^rgb\((\d{1,3})\,(\d{1,3})\,(\d{1,3})\)$/i.test(value)) {
			temp = value.match(/^rgb\((\d{1,3})\,(\d{1,3})\,(\d{1,3})\)$/i);
			rgb = {
				r: parseInt(temp[1]),
				g: parseInt(temp[2]),
				b: parseInt(temp[3])
			...
			rgb = this.colorAIP.hex2rgb(value);
		}
	}
	
	is_check = is_check && rgb instanceof Object;
	...
	new_value = this.colorAIP.rgb2hex({ r:rgb.r, g:rgb.g, b:rgb.b });
...
dhtmlXColorPicker.prototype._refreshContrast = function() {
	...
	if (ieV && ieV<=9) {
		...
		this._controllerNodes.contrastArea.style.backgroundImage = "linear-gradient(to top, rgb("+rgb_top.join(",")+"), rgb("+rgb_bot.join(",")+"))";
	}
};
