...
$(document).ready(function(){
	...
	gridDtl.addHeader({name:"구분",     colId:"przPun",     width:"7", align:"center",  type:"combo"});
...
function doOnDtlRowSelect(id,ind){
	...
	var seqValue = gridDtl.setCells(id,7).getValue();
	if(seqValue == ""){
		gridDtl.setCells(id,7).setValue(no);
	}
}
function fn_search(){
	gridDtl.clearAll();
...
function fn_save(){
   ...
   if (jsonStr == null || jsonStr.length <= 0) return;         		
       ...
       $.ajax({
          url : "/erp/pers/pers/rewardDataS/gridDtlSave",
          type : "POST",
...
function fn_refreshGrid(id){
	var obj={};
	  obj.compId = gridDtl.setCells(id,6).getValue();
	  obj.empNo = gridDtl.setCells(id,5).getValue();
	  fn_loadGridRightList(obj);
}
